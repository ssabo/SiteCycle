...
struct CSVImporter {
    ...
    static func parseCSV(_ csv: String) -> [[String]] {
        ...
        // Append last field/row
        currentRow.append(currentField)
        if !currentRow.isEmpty {
            rows.append(currentRow)
        }

        // Remove trailing [""] row from trailing newline
        if rows.last == [""] {
            rows.removeLast()
        }
    ...
    private static func parseAndImport(csv: String, context: ModelContext) throws -> Int {
        ...
        for row in dataRows {
            ...
            let dateStr = row[0]
            let locationName = row[1]
            let durationStr = row[2]
            let note = row[3]

            guard let startTime = dateFormatter.date(from: dateStr) else { continue }

            let location: Location
            if let cached = locationCache[locationName] {
                location = cached
            } else {
                let newLocation = makeLocation(from: locationName, sortOrder: sortOrder)
                context.insert(newLocation)
                locationCache[locationName] = newLocation
                sortOrder += 1
            ...
            if durationStr.isEmpty {
                endTime = nil
            } else if let hours = Double(durationStr) {
